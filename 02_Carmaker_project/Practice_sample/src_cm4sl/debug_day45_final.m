...
hw=2.48*0.5+1.5; hl=11.5*0.5+1.5;
for i=1:7
    px=trucks(i,1); py=trucks(i,2); yaw=deg2rad(trucks(i,3));
    y = stamp(y, px, py, yaw, hl, hw);
end
hw0=1.97*0.5; hl0=4.47*0.5;
y = stamp(y, t00(1), t00(2), deg2rad(t00(3)), hl0, hw0);
...
plot([0 100],[0 0],'g--');
title({'FINAL: trucks T01..T07 inflation +2.0m, T00 (car) inflation +1.0m','road\_y\_max=0'});
saveas(gcf, fullfile(fileparts(mfilename('fullpath')), 'debug_day45_final.png'));
end

function y = stamp(y, px, py, yaw, hl, hw)
c=cos(yaw); s=sin(yaw);
