"""VehicleLat — kinematic bicycle 차량 모델. Fixed simulation environment.

이 파일은 검증 환경의 일부입니다. 수정하지 마세요.

Dynamics (per step):
    δ_clipped  = clip(delta, -0.5, +0.5)        # rad
    yawrate    = vx / (L + kv · vx²) · δ_clipped
    Yaw_{k+1}  = Yaw_k + dt · yawrate
    X_{k+1}    = X_k + vx · dt · cos(Yaw_{k+1})
    Y_{k+1}    = Y_k + vx · dt · sin(Yaw_{k+1})
"""
