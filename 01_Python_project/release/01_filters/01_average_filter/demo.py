"""Average Filter demo — N(5, 1) 노이즈 신호의 평균 추정 (plotly, 브라우저 렌더).

실행 전 `average_filter.py` 의 `# TODO` 를 구현해야 동작합니다.
...
def main() -> None:
    ...
    af = AverageFilter()
    estimate = np.array([af.step(s) for s in samples])
    ...
    fig.add_trace(
        go.Scatter(y=estimate, mode="lines", name="estimate",
                   line=dict(color="blue", width=2))
    )
    fig.update_layout(
        title="Average Filter — recursive cumulative mean",
