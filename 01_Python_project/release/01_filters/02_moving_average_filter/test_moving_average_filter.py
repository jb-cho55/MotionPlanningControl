"""MovingAverageFilter regression — behavioral spec (requirements level).

알고리즘 형태 (deque sliding / cumulative subtract / 다른 방식) 는 자유.
...
def test_noisy_tracking_rms_within_spec():
    """상수 truth=5 + N(0, 1) 노이즈 1만 표본, window=20: warm-up 이후 RMS < 0.4.
    ...
    estimates = np.array([maf.step(s) for s in samples])
