"""RRT* 시각화 — node 추가 + rewire + tree edge 누적을 Rerun 2D 로 재생.

`--seed S` / `--eta E` / `--radius R` / `--max-iter N` 로 실험 가능. `--skip N` 으로
frame subsample (기본 10). 재생: 같은 폴더 ../simulator_search.py.

디버그 신호: rrt_star() 가 매 iteration(reject 포함) `goal_dist`·`rejected`·
`tree_size`·`near_count`·`rewire_count` 를 수집해 record 의 `debug_scalars` 로
저장한다 (x축 = iteration). viewer 기본 화면엔 안 보이며, entity 패널에서
...
def main() -> None:
    ...
    parser.add_argument("--radius", type=float, default=8.0,
                        help="choose-parent·rewire 반경 (기본 8.0).")
    ...
    raw_steps: list[dict] = []
    dbg = DebugSignals()  # 디버그 신호 수집기 — rrt_star() 가 매 iteration 채운다
    ...
    path = rrt_star(START, GOAL, OBSTACLES, GRID_SIZE,
                    max_iter=args.max_iter, eta=args.eta,
                    search_radius=args.radius, seed=args.seed,
                    on_step=on_step, dbg=dbg)
    if not path:
    ...
    for i in range(0, len(raw_steps), skip):
        ...
        frames.append({
            ...
            "open": [],  # RRT* 도 open list (priority queue) 없음.
            "expanded": [step["current"] for step in batch],
            "new_edges": [step["edge"] for step in batch],
            "iterations": [step["iteration"] for step in batch],  # add/rewire 별 iteration
        })

    record = {
        "schema_version": 1,
        "module": "04_path_planning/08_rrt_star",
        "kind": "search",
        "grid_size": GRID_SIZE,
        ...
        # 디버그 신호 — rrt_star() 가 매 iteration(reject 포함) 수집한 시계열
        # (t = iteration). 기본 blueprint 미포함 — viewer entity 패널에서
        # /debug/<name> 을 TimeSeriesView 로 추가해 본다.
    ...
    print(f"[record] saved → {out} (seed={args.seed}, eta={args.eta}, "
          f"radius={args.radius}, {len(raw_steps)} steps → {len(frames)} frames "
